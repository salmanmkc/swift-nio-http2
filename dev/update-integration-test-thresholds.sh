...
THRESHOLDS_DIR="${THRESHOLDS_DIR:-$REPO_ROOT/IntegrationTests/tests_01_allocation_counters/Thresholds}"
...
fetch_checks_for_workflow() {
    ...
    "$GH_BIN" --repo "$repo" run view "$run" --json jobs --jq '.jobs[] | select(.name | contains("Integration Tests")) | "\(.name) (ID \(.databaseId))"'
}
